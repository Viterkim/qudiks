...
Packages the existing release build and writes an archive and SHA-256 checksum
to dist/. Pass --build to rebuild first. Upload both files to a GitHub release.
USAGE
}
...
command -v sha256sum >/dev/null || { echo "sha256sum is required to package the release binary" >&2; exit 1; }
if [[ -n $build ]]; then
...
echo "  $repo_root/dist/$asset.sha256"
