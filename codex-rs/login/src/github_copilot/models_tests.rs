...
#[test]
fn chooses_gpt_6_sol_for_a_new_profile() {
    let models = [
        model("gpt-5.3-codex"),
        model("grok-4.7"),
        model("gpt-6-sol"),
    ...
    assert_eq!(
        choose_model(&models, Some("gpt-5.3-codex")),
        Some("gpt-5.3-codex".to_string())
    );
}
...
async fn accepts_an_enterprise_host_when_the_domain_is_known() {
    let api_base_url = "https://copilot-api.github.example.com";
    assert!(assert_trusted_copilot_api_url(api_base_url, /*enterprise_domain*/ None).is_err());
    assert_eq!(
        assert_trusted_copilot_api_url(api_base_url, Some("github.example.com"))
            .expect("enterprise host is trusted")
            .as_str(),
...
#[test]
fn does_not_choose_a_chat_only_preference_without_a_usable_model() {
    assert_eq!(choose_model(&[chat_only("claude")], Some("claude")), None);
}
