...
#[clap(
    ...
    // the generic `qudiks` command name that users run.
    bin_name = "qudiks",
    override_usage = "qudiks [OPTIONS] [PROMPT]\n       qudiks [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `qudiks mcp`: `qudiks`, `qudiks exec`, `qudiks review`, `qudiks resume`, `qudiks queue`, `qudiks archive`, `qudiks delete`, `qudiks unarchive`, `qudiks fork`, `qudiks mcp`, `qudiks sandbox`, and `qudiks debug prompt-input`."
        ),
    }
}
