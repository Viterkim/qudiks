...
Ask clarifying questions only when the missing information matters and cannot be reasonably inferred. Respect a user-specified location; otherwise create discoverable skills in the active Qudiks home’s `skills` directory (`QUDIKS_HOME`, then `CODEX_HOME`, then `~/.qudiks`).
...
```bash
scripts/init_skill.py my-skill --path "${QUDIKS_HOME:-${CODEX_HOME:-$HOME/.qudiks}}/skills"
scripts/init_skill.py my-skill --path "${QUDIKS_HOME:-${CODEX_HOME:-$HOME/.qudiks}}/skills" --resources references
```
