...
- In built-in tool mode, Qudiks saves generated images under the active Qudiks home (`QUDIKS_HOME`, then `CODEX_HOME`, then `~/.qudiks`) by default.
- Do not describe or rely on OS temp as the default built-in destination.
- Do not describe or rely on a destination-path argument (if any) on the built-in `image_gen` tool. If a specific location is needed, generate first and then move or copy the selected output from the active Qudiks home's `generated_images` directory.
- Save-path precedence in built-in mode:
  1. If the user names a destination, move or copy the selected output there.
  2. If the image is meant for the current project, move or copy the final selected image into the workspace before finishing.
  3. If the image is only for preview or brainstorming, render it inline; the underlying file can remain in the active Qudiks home.
- Never leave a project-referenced asset only in the active Qudiks home.
...
14. For preview-only work, render the image inline; the underlying file may remain in the active Qudiks home's `generated_images` directory.
15. For project-bound work, move or copy the selected artifact into the workspace and update any consuming code or references. Never leave a project-referenced asset only in the active Qudiks home's `generated_images` directory.
